clc;
pzstat = input('enter a puzzle in 3 x 3 form   ');
pzrstat = [8 1 6;4 5 3;7 0 2];
intv000 = size(pzstat);
if not(all(intv000 == [3 3]))
    ...
    while intv001 == 1;
        intv0ct = intv0ct + 1;
        [movedisplay, intv00f, intv00g, intv00h] =  gskpzget(pzinl,1);
        [intv002, intv003] = find(movedisplay==0);
        pzinl = [];
        if intv00h ~= 0
% % % % % % left
            if intv003 > 1
                movedisplay1 = movedisplay;
                movedisplay1(intv002,intv003) = movedisplay(intv002,intv003 - 1);
                movedisplay1(intv002,intv003 - 1) = 0;
                [intv0h1,intv0h2] = gskcostastar(movedisplay1,pzrstat);
                intv00h = intv0h1 + intv0h2;
                intv0g1 = intv00g + 1;
                intv00f = intv0g1 + intv00h;
                [pzinl] =  gskpzjoin(pzinl,movedisplay1,intv00f,intv0g1,intv00h);
            end
% % % % % % top
            if intv002 > 1
                movedisplay1 = movedisplay;
                movedisplay1(intv002,intv003) = movedisplay(intv002-1,intv003);
                movedisplay1(intv002-1,intv003) = 0;
                [intv0h1,intv0h2] = gskcostastar(movedisplay1,pzrstat);
                intv00h = intv0h1 + intv0h2;
                intv0g1 = intv00g + 1;
                intv00f = intv0g1 + intv00h;
...
% % % % % % selecting new node
            ...
            [movedisplay, intv00f, intv00g, intv00h] =  gskpzget(intv003,1);
            [intv002, intv003] = find((pzrhis(:,10)==intv00f) & (pzrhis(:,13) == 0));
%             intv002 = intv002(1,1);
            [movedisplay, intv00f, intv00g, intv00h] =  gskpzget(pzrhis,intv002(1,1));
            intv0nd = pzrhis(intv002(1,1),15);
            pzinl = [];
            [pzinl] =  gskpzjoin(pzinl,movedisplay,intv00f,intv00g,intv00h);
...
% % Display of steps
    ...
    for intv000 = 1 : intvcnt
        [movedisplay, intv00f, intv00g, intv00h] =  gskpzget(pzhis,pzshow(intvcnt-intv000+1));
        intv000
        movedisplay
    end
end